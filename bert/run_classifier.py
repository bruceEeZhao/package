...
class emotion(DataProcessor):
  ...
  def get_test_examples(self, data_dir):
    """See base class."""
    examples = list()
    dataFrame = pd.read_csv('./datas/preprocessed_test_data.csv').astype('str')
    for index, item in dataFrame.iterrows(): 
      guid = tokenization.convert_to_unicode(item[0])      # id
      text_a = tokenization.convert_to_unicode(item[3])    # title
      examples.append(
        InputExample(guid=guid, text_a=text_a, label='1'))
    return examples 
